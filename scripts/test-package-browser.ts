...
async function assertPage(page: Page, heading: string, title: string) {
  ...
  assert.equal(await page.locator('head meta[name="twitter:image"]').getAttribute('content'), 'http://localhost/preview/opengraph-image.png')
  assert.equal(await page.locator('head link[rel=icon]').getAttribute('href'), '/preview/icon.svg')
  assert.equal(await page.locator('head meta[property="og:image"]').getAttribute('content'), 'http://localhost/preview/opengraph-image.png')
}
